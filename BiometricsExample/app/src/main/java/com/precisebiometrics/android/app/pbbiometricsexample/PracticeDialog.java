...
/**
 * Practice fingerprint swipe dialog.
 * This is a reference example of a {@link PBBiometryGUI} to handle
 * the events gotten from image capture in order to show a user
 * the swiped fingerprint image together with instructions and other
 * information.
 */
public class PracticeDialog extends Dialog implements PBBiometryGUI {
    ...
    /** Text view for showing the latest fingerprint images quality value. */
    private TextView qualityValue;
    /** Text view for showing the latest fingerprint images area value. */
    ...
    /**
     * Creates a dialog that shows the GUI used for fingerprint swipe practice.
     * 
     * @param activity The context of the dialog.
     * @param bio The biometric API, that can be used for cancellation.
     */
    public PracticeDialog(Activity activity, PBBiometry bio) {
        super(activity);
        ...
        cancelButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    PracticeDialog.this.bio.cancel();
        ...
        activity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                case PBEventAlertImageCaptured:
                    ...
                    // Clear the error text now when we are on the next image
                    errorText.setText("");
                    break;
    ...
    @Override
    public void displayQuality(final int imageQuality, final int area, 
                               int imageQualityThreshold, int areaThreshold)
            throws PBBiometryException {
        activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                qualityValue.setText("" + imageQuality);
                areaValue.setText("" + area + " mm^2");
            }
        });
    }
}
